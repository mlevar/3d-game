  ...
  protected void checkCollisions() {
    ...
    for (int i=0; i<balls.length-1; i++)
      for (int j=i+1; j<balls.length; j++) {
        double r2 = Math.pow((balls[i].m_nX - balls[j].m_nX),2) + Math.pow(balls[i].m_nY - balls[j].m_nY, 2) + Math.pow(balls[i].m_nZ - balls[j].m_nZ, 2);
        if (r2 < Math.pow(balls[i].radius + balls[j].radius, 2)) {
          float[] s = balls[i].speed;
