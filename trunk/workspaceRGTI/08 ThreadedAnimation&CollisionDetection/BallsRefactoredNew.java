...
public class BallsRefactoredNew extends BaseWindow 
{
  BitmapText text;
 ...
 // select modulate to mix texture with color for shading; GL_REPLACE, GL_MODULATE ...
  ...
  protected void initializeModels()
  {
    ...
    for (int i=0; i<balls.length; i++) {
       balls[i] = new BallNew((float)Math.random()*0.2f+0.1f);
  ...
  protected void checkCollisions() {
    ...
    for(int i=0; i<balls.length-1; i++)
      for (int j=i+1; j<balls.length; j++) {
        if(balls[i].colidesWith(balls[j])) {
          balls[i].reverseSpeed();
          balls[j].reverseSpeed();
        }
...
//   HUD & Text render
  ...
  public static void main(String[] args) {
    (new BallsRefactoredNew()).execute();
  }  
}
